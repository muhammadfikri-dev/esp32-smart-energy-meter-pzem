...
void setup() {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *r){
        r->send(200, "text/html", "<h2>Laksanasoft Smart Energy Meter (Live WebSocket Active)</h2>");
    });
    server.begin();
...
void loop() {
    ...
    if (!mqttClient.connected()) {
        mqttClient.connect("Laksanasoft-EnergyMeter-PZEM");
    }
    mqttClient.loop();
    ...
    char buffer[384];
    serializeJson(doc, buffer);
    mqttClient.publish("laksanasoft/energy/telemetry", buffer);
